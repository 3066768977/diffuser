...
def get_linear_schedule_with_warmup_two_stage(optimizer, num_warmup_steps_stage_one, num_training_steps_stage_one,
                                              num_warmup_steps_stage_two, num_training_steps_stage_two,
                                              stage_one_lr_scale, last_epoch=-1):
    def lr_lambda(current_step: int):
        if current_step < num_training_steps_stage_one:
            ...
            progress = float(current_step - num_warmup_steps_stage_one) / float(
                max(1, num_training_steps_stage_one - num_warmup_steps_stage_one))
            return max(
                0.0, float(num_training_steps_stage_one - current_step) / float(
                    max(1, num_training_steps_stage_one - num_warmup_steps_stage_one)) * stage_one_lr_scale
            )
        else:
            ...
            progress = float(current_step - num_warmup_steps_stage_two) / float(
                max(1, num_training_steps_stage_two - num_warmup_steps_stage_two))
            # return max(0.0, 0.5 * (1.0 + math.cos(math.pi * float(0.5) * 2.0 * progress)))
            return max(
                0.0, float(num_training_steps_stage_two - current_step) / float(
                    max(1, num_training_steps_stage_two - num_warmup_steps_stage_two))
            )
...
class DiffusionNERTrainer(BaseTrainer):
    ...
    def train(self, train_path: str, valid_path: str, types_path: str, input_reader_cls: BaseInputReader):
        ...
        for epoch in range(args.epochs):
            ...
            if (not args.final_eval or (epoch == args.epochs - 1)) and self.record and (
                    (epoch % args.eval_every_epochs) == 0 or (epoch == args.epochs - 1)):
    ...
    def _train_epoch(self, model: torch.nn.Module, compute_loss: Loss, optimizer: Optimizer, dataset,
        ...
        for batch in tqdm(data_loader, total=total, desc='Train epoch %s' % epoch):
            ...
            batch_loss = compute_loss.compute(outputs, gt_types=batch['gt_types'], gt_spans=batch['gt_spans'],
                                              entity_masks=batch['entity_masks'], epoch=epoch, batch=batch)
