...
class DiffusionNERLoss(Loss):
    ...
    def compute(self, output, gt_types, gt_spans, entity_masks, epoch, batch = None):
        ...
        # Check if pred_spans and pred_logits are available and have correct dimensions
        if "pred_spans" not in output or len(output["pred_spans"].shape) != 3:
            print("Error: pred_spans not found or has incorrect dimensions")
            return 0

        if "pred_logits" not in output:
            print("Error: pred_logits not found")
            return 0

        # Assignments
        pred_logits = output["pred_logits"]
        pred_left = output["pred_spans"][:, :, 0]
        pred_right = output["pred_spans"][:, :, 1]

        # Prepare outputs dictionary
        outputs = {"pred_logits": pred_logits, "pred_left": pred_left, "pred_right": pred_right, "token_mask": batch["token_masks"]}
        ...
        tversky_loss_dict = self.criterion.tversky_loss(outputs, targets, indices, num_spans, alpha=0.5, beta=0.5)
        train_loss += sum(tversky_loss_dict[k] * self.weight_dict.get(k, 1.0) for k in tversky_loss_dict.keys())
...
class Criterion(nn.Module):
    ...
    def tversky_loss(self, outputs, targets, indices, num_spans, alpha=0.5, beta=0.5):
        # 您需要根据模型输出和目标来计算 true positives, false positives, 和 false negatives
        # 这里是计算方法的一个简化示例，具体实现可能需要根据您的模型结构进行调整
        pred_logits = outputs['pred_logits']
        gt_labels = targets['labels']

        # 将预测和真实标签转换为 one-hot 编码
        pred_one_hot = F.one_hot(pred_logits.argmax(dim=-1), self.entity_type_count)
        gt_one_hot = F.one_hot(gt_labels, self.entity_type_count)

        # 计算 true positive, false positive 和 false negative
        tp = (pred_one_hot & gt_one_hot).sum(dim=1).float()
        fp = (pred_one_hot & (~gt_one_hot)).sum(dim=1).float()
        fn = ((~pred_one_hot) & gt_one_hot).sum(dim=1).float()

        # 计算 Tversky Loss
        tversky_index = tp / (tp + alpha * fp + beta * fn)
        tversky_loss = 1 - tversky_index.mean()

        return {'tversky_loss': tversky_loss}
