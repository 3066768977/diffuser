...
class DiffusionNER(PreTrainedModel):
    ...
    def __init__(
        ...
        self.bilstm = nn.LSTM(input_size=config.hidden_size,
                              hidden_size=config.hidden_size,  # 更改hidden_size
                              num_layers=3,  # 增加层数
                              bidirectional=True,
                              batch_first=True)
        
        self.hidden2tag = nn.Linear(2048, entity_type_count)

        # 修改CRF层参数
        self.crf = CRF(num_tags=entity_type_count, batch_first=True)  # 可以尝试添加更多的CRF参数
    ...
    def forward(self, 
        ...
        h_token, h_token_lstm = self.backbone(encodings, context_masks, token_masks, pos_encoding, seg_encoding, context2token_masks)
        lstm_out, _ = self.bilstm(h_token)
        lstm_out = self.hidden2tag(lstm_out)

        max_seq_length = 70  # Desired sequence length
        current_seq_length = lstm_out.shape[1]

        # Adjust LSTM output to match the max_seq_length
        if current_seq_length > max_seq_length:
            lstm_out = lstm_out[:, :max_seq_length, :]
        elif current_seq_length < max_seq_length:
            padding = max_seq_length - current_seq_length
            lstm_out = F.pad(lstm_out, (0, 0, 0, padding))

        # Ensure entity_types and token_masks match lstm_out in sequence length
        if entity_types is not None and token_masks is not None:
            if entity_types.shape[1] != max_seq_length:
                if entity_types.shape[1] > max_seq_length:
                    entity_types = entity_types[:, :max_seq_length]
                else:
                    padding = max_seq_length - entity_types.shape[1]
                    entity_types = F.pad(entity_types, (0, padding), value=-1)  # Assuming -1 is an ignore index

            if token_masks.shape[1] != max_seq_length:
                if token_masks.shape[1] > max_seq_length:
                    token_masks = token_masks[:, :max_seq_length]
                else:
                    padding = max_seq_length - token_masks.shape[1]
                    token_masks = F.pad(token_masks, (0, padding))

        # Rest of your code for CRF loss computation
        if self.training:
            # Additional parameter updates and logging if needed
            ...
            # Compute CRF loss
            crf_loss = -self.crf(lstm_out, entity_types, mask=token_masks.byte(), reduction='mean')
            return crf_loss

        # Inference mode
        return self.ddim_sample(h_token, h_token_lstm, token_masks)
    ...
    def backbone(self, 
        ...
        if self.lstm_layers > 0:
            #自己添加4行
            token_lengths = token_masks.sum(dim=1)  # 计算每个序列的实际长度
            h_token_packed = nn.utils.rnn.pack_padded_sequence(h_token, token_lengths.cpu(), batch_first=True, enforce_sorted=False)  # 打包序列
            h_token_lstm, (h, c) = self.lstm(h_token_packed)
            h_token_lstm, _ = nn.utils.rnn.pad_packed_sequence(h_token_lstm, batch_first=True)  # 解包序列
